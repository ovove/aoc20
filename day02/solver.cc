...
namespace {
static const std::regex passwd_policy_record_regex{
    R"(^ *([1-9][0-9]*) *- *([1-9][0-9]*) *([a-z]) *: *([a-z]+) *$)"};
}
...
bool sled_validate_password_policy_record(const PasswordPolicyRecord& password_policy_record)
{
    ...
    const auto required_char_count =
        std::count(std::begin(password), std::end(password), required_char);
